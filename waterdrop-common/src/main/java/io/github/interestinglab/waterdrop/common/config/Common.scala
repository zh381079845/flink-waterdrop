...
object Common {
  ...
  private var mode = Some("client")

  def isModeAllowed(mode: String): Boolean = ALLOWED_MODES.foldRight(false)((m, lastResult) => {
    lastResult match {
      case true => true
  ...
  /**
    ...
    * */
  def setDeployMode(mode: String): Boolean = isModeAllowed(mode) match {
    case true => {
      this.mode = Some(mode)
      true
    }
  ...
  def getDeployMode: Option[String] = this.mode
  ...
  /**
    ...
    * */
  def appRootDir: Path = this.mode match {
    case Some("client") => {
  ...
  /**
    * Plugin Root Dir
    * */
  def pluginRootDir: Path = Paths.get(appRootDir.toString, "plugins")

  /**
    * Get specific plugin dir
    * */
  def pluginDir(pluginName: String): Path = Paths.get(pluginRootDir.toString, pluginName)

  /**
    * Get files dir of specific plugin
    * */
  def pluginFilesDir(pluginName: String): Path = Paths.get(pluginDir(pluginName).toString, "files")

  /**
    * Get lib dir of specific plugin
    * */
  def pluginLibDir(pluginName: String): Path = Paths.get(pluginDir(pluginName).toString, "lib")
}