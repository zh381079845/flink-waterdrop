...
public class SchemaUtil {
    ...
    private static void getOrcSchema(Schema schema, JSONObject json) {
        // 调用 getTypeInformation 解析 JSON 结构，生成 RowTypeInfo
        RowTypeInfo typeInfo = getTypeInformation(json);
        // 遍历字段名和类型，添加到 Schema 中
...
//    }
    ...
    private static void getParquetSchema(Schema schema, JSONObject json) {
        // 与 Orc 的逻辑一致，直接复用 getTypeInformation 解析 JSON
        RowTypeInfo typeInfo = getTypeInformation(json);
        String[] fieldNames = typeInfo.getFieldNames();
        TypeInformation<?>[] fieldTypes = typeInfo.getFieldTypes();
        for (int i = 0; i < fieldNames.length; i++) {
